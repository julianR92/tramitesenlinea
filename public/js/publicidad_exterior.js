...
doc.addEventListener("DOMContentLoaded", function () {
    ...
    if (e.target.matches(".btnValidar")) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            doc.getElementById("dir_solicitante").value =
              response.persona.PersonaDir;
            doc.getElementById("PersonaTel").value =
              response.persona.PersonaTel;
            doc.getElementById("PersonaMail").value =
              response.persona.PersonaMail;
            // doc.getElementById("PersonaBarrio").value =
            //     response.persona.PersonaBarrio;
            $("#PersonaBarrio")
              .val(response.persona.PersonaBarrio)
              .trigger("change");

            doc.getElementById("PersonaTip").value =
              response.persona.PersonaTip;
            validarTipoPersona(response.persona.PersonaTip);
            doc.getElementById("PersonaNombre").value =
              response.persona.PersonaNombre;
            doc.getElementById("PersonaApe").value =
              response.persona.PersonaApe;
            doc.getElementById("PersonaRazon").value =
              response.persona.PersonaRazon;
  ...
  doc.addEventListener("change", (e) => {
    ...
    if (e.target.matches("#publicidad_modalidad")) {
      ...
      if (modalidad) {
          if (modalidad == "comerciales") {
          const divSubModalidad = doc.getElementById("divSubModalidad");
          divSubModalidad.classList.remove("d-none");
        } else {
          divSubModalidad.classList.add("d-none");
          const pubSubModalidad = document.getElementById("pub_sub_modalidad");
          if (pubSubModalidad) {
            pubSubModalidad.value = "";
          }
          $.ajax({
            ...
            error: function (xhr) {
              $("#divVistasModalidades").html(
                "<p>Ha ocurrido un error al cargar la vista.</p>"
              );
            },
          });
        }
    ...
    if (e.target.matches("#pub_sub_modalidad")) {
        let modalidad = e.target.value;
          ...
          if (response.success) {
            ...
            setInterval(() => {
              window.location.href =
                "/publicidad-exterior/solicitud/" + response.persona.PersonaDoc;
            }, 2000);
          } else {
            $("#loadMe").modal("hide");
          ...
          adjuntos = `
               <div class="col-md-6">
                  ...
                  <div class="form-group">
                     <input class="documentos_adjuntos form-control" id="${el.codigo_adjunto}" accept="application/pdf" name="${el.codigo_adjunto}" type="file" data-overwrite-initial="true"  ${requiredAttr}>
                  </div>
               </div>`;

...
class PublicidadAdmin extends Controller
{
	...
	public function detalle($id)
	{
		...
		$sql = "SELECT estado_id AS estado_id,nov.id AS novedad_id,estado,titulo_estado,novedad,opcion,estado_sig,finaliza
	...
	public function AgregarNovedad(Request $req)
	{
		...
		if (!empty($solicitud)) {
			...
			try {
				...
				$estado_siguiente = DB::select("SELECT estado,titulo_estado,dependencia,novedad,opcion,estado_sig FROM publicidad_config_estados pce INNER JOIN publicidad_config_novedades AS pcn ON pce.id=pcn.estado_id  WHERE pce.id=$estado_sig and tipo_publicidad='$solicitud->tipo_publicidad' LIMIT 1;");

				$proximo_estado = $estado_siguiente[0]->titulo_estado ?? 'Finalizado';
		...
		Alert::success('Acción Exitosa', "Siguiente estado: " . $proximo_estado . " radicado N° " . $solicitud->radicado);
	...
	private function generarLiquidacion($personas, $solicitud, $req)
	{
		...
		$conexion = DB::connection('sqlsrv3');

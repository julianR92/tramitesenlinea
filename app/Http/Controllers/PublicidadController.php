...
class PublicidadController extends Controller
{
    ...
    public function storeVallas(Request $request)
    {
        DB::beginTransaction();
    ...
    public function storeAvisos(Request $request)
    {
        ...
        $request->validate(
            [
                ...
                'publicidad_modalidad' => 'required|string',
                'tipo_publicidad' => 'required|string',
                'fecha_renovacion' => 'required_if:tipo_publicidad,RENOVACION|date|nullable',
                'fecha_vencimiento' => 'required_if:tipo_publicidad,RENOVACION|date|nullable',

                'comerciales_direccion_elemento1' => [
                    'required',
                ],

                'comerciales_numero_elementos' => 'required|integer|min:1|max:2',
                // Elemento 1
                'comerciales_alto_elemento1' => 'required|numeric|max:999999',
                'comerciales_ancho_elemento1' => 'required|numeric|max:999999',
                'comerciales_area_total_elemento1' => 'required|numeric|max:999999',
                'comerciales_alto_fachada1' => 'required|numeric|max:999999',
                'comerciales_ancho_fachada1' => 'required|numeric|max:999999',
                'comerciales_area_total_fachada1' => 'required|numeric|max:999999',

                // Elemento 2 (requerido solo si hay 2 elementos)
                'comerciales_alto_elemento2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',
                'comerciales_ancho_elemento2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',
                'comerciales_area_total_elemento2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',
                'comerciales_alto_fachada2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',
                'comerciales_ancho_fachada2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',
                'comerciales_area_total_fachada2' => 'required_if:comerciales_numero_elementos,2|numeric|max:999999|nullable',

                // Archivos PDF (10MB máximo)
                'plano_digitalizado' => 'required|file|mimes:pdf|max:10240',
                'autorizacion_propietario' => 'required|file|mimes:pdf|max:10240',
                'rut_o_camara' => 'required|file|mimes:pdf|max:10240',
                // Confirmaciones tipo "checkbox"
                'tratamiento_datos' => 'required',
                'acepto_terminos' => 'required',
                'confirmo_mayorEdad' => 'required',
                'confirmo_politica' => 'required',
                'compartir_informacion' => 'required',
                'comerciales_tipo_valla' => 'required',

            ],
            [

                'fecha_renovacion.required_if' => 'La fecha de renovación es obligatoria cuando el tipo de publicidad es RENOVACIÓN.',
                'fecha_vencimiento.required_if' => 'La fecha de vencimiento es obligatoria cuando el tipo de publicidad es RENOVACIÓN.',

                'comerciales_direccion_elemento1.required' => 'La dirección del elemento es obligatoria.',
                'comerciales_direccion_elemento1.regex' => 'La dirección no tiene un formato válido. Ej: CARRERA 12 #12-12.',

                'comerciales_numero_elementos.required' => 'Debe indicar cuántos elementos tiene.',
                'comerciales_numero_elementos.integer' => 'El número de elementos debe ser un número.',
                'comerciales_numero_elementos.max' => 'Solo se permiten hasta 2 elementos.',

                'comerciales_alto_elemento2.required_if' => 'El alto del segundo elemento es obligatorio cuando hay 2 elementos.',
                'comerciales_ancho_elemento2.required_if' => 'El ancho del segundo elemento es obligatorio cuando hay 2 elementos.',
                'comerciales_area_total_elemento2.required_if' => 'El área total del segundo elemento es obligatoria cuando hay 2 elementos.',
                'comerciales_alto_fachada2.required_if' => 'El alto de fachada 2 es obligatorio cuando hay 2 elementos.',
                'comerciales_ancho_fachada2.required_if' => 'El ancho de fachada 2 es obligatorio cuando hay 2 elementos.',
                'comerciales_area_total_fachada2.required_if' => 'El área total de fachada 2 es obligatoria cuando hay 2 elementos.',

                'plano_digitalizado.required' => 'Debe adjuntar el plano digitalizado en formato PDF.',
                'plano_digitalizado.mimes' => 'El plano digitalizado debe ser un archivo PDF.',
                'plano_digitalizado.max' => 'El plano digitalizado no debe superar los 10MB.',

                'autorizacion_propietario.required' => 'Debe adjuntar la autorización del propietario.',
                'autorizacion_propietario.mimes' => 'La autorización del propietario debe ser un archivo PDF.',
                'autorizacion_propietario.max' => 'La autorización del propietario no debe superar los 10MB.',

                'rut_o_camara.required' => 'Debe adjuntar el RUT o cámara de comercio.',
                'rut_o_camara.mimes' => 'El archivo debe ser un PDF.',
                'rut_o_camara.max' => 'El archivo no debe superar los 10MB.',

                'tratamiento_datos.required' => 'Debe aceptar el tratamiento de datos.',
                'acepto_terminos.required' => 'Debe aceptar los términos y condiciones.',
                'confirmo_mayorEdad.required' => 'Debe confirmar que es mayor de edad.',
                'confirmo_politica.required' => 'Debe aceptar la política de privacidad.',
                'compartir_informacion.required' => 'Debe confirmar si acepta compartir la información.',
        ...
        try {
            ...
            $publicidad->numero_elementos = $request->comerciales_numero_elementos;
            $publicidad->esquinero = null;
            $publicidad->estado_solicitud = "radicado";
            $publicidad->novedad = "Revisión de documentos radicados";
            $publicidad->dependencia = "interior";
            ...
            if ($publicidad->id > 0) {
                ...
                for ($i = 1; $i <= $publicidad->numero_elementos; $i++) {
                    ...
                    $detalle->alto_elemento = $request->input($modalidad . "_alto_elemento" . $i, null);
                    $detalle->ancho_elemento = $request->input($modalidad . "_ancho_elemento" . $i, null);
                    $detalle->numero_caras = 1;
                    $detalle->area_total_elemento = $request->input($modalidad . "_area_total_elemento" . $i, null);
                    $detalle->ancho_fachada = $request->input($modalidad . "_ancho_fachada" . $i, null);
                    $detalle->alto_fachada = $request->input($modalidad . "_alto_fachada" . $i, null);
                    $detalle->area_total_fachada = $request->input($modalidad . "_area_total_fachada" . $i, null);
                    $detalle->descripcion_elemento = $request->input($modalidad . "_descripcion_elemento" . $i, null);
                    $detalle->direccion_elemento = $request->comerciales_direccion_elemento1;
                    $detalle->caracteristicas_vehiculo = $request->input($modalidad . "_caracteristicas_vehiculo" . $i, null);
                    $detalle->tipo_vehiculo = $request->input($modalidad . "_tipo_vehiculo" . $i, null);
                    $detalle->placa_vehiculo = $request->input($modalidad . "_placa_vehiculo" . $i, null);
                    $detalle->save();
